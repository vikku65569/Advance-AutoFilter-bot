        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file['file_size'])}] [{'DOC' if file['file_type'].lower() == 'PDF' else 'AUDIOBOOK'}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file['file_name'].split()))}", callback_data=f'{pre}#{file["file_id"]}'
                ),
            ]
            for file in files
        ]
